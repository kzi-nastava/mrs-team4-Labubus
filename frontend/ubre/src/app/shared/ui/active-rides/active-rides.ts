...
export class ActiveRides {
  ...
  onQueryChange(qurey : RideQueryDto) {
    this.rideService.clearActive();
    this.rideService.fetchActive(qurey, Math.ceil((window.screen.height * 0.7 - 126) / window.devicePixelRatio / 171))
  ...
  ngOnInit() {
    this.rides$ = this.rideService.active$;
